...
const OrganizerScannersScreen = ({ navigation }) => {
  const { state } = useAuth();
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const [scannersRes, eventsRes] = await Promise.all([
        organizerListScannersApi(),
        organizerListEventsApi(),
      ]);
      setScanners(scannersRes?.data?.info || []);
  ...
  const onCreate = async () => {
    ...
    try {
      setIsLoading(true);
      const res = await organizerCreateScannerApi({ name: trimmedName, email: trimmedEmail, password });
      const created = res?.data?.info || res?.data;
      const createdId = created?._id || created?.id;
      if (selectedEventId && createdId) {
        if (canValidateAll) {
          await organizerAssignScannerWithTypesApi({
            eventId: selectedEventId,
            scannerId: createdId,
            allowAll: true,
          });
          setStatus("Scanner creado y asignado.");
        } else {
          setStatus("Scanner creado. Asignacion pendiente.");
        }
      } else {
  ...
  const resolveScannerId = (scanner) =>
    String(scanner?._id || scanner?.id || scanner?.email || scanner?.name || "");
  const resolveEventName = (scanner) => {
    const direct =
      scanner?.event?.name ||
      scanner?.eventName ||
      scanner?.eventid?.name ||
      scanner?.evento ||
      "";
    if (direct) return direct;
    const eventId = scanner?.eventId || scanner?.eventid || scanner?.assignedEventId;
  ...
  const resolveStatus = (scanner) => {
    const raw = scanner?.status ?? scanner?.state ?? scanner?.isActive ?? scanner?.active;
  ...
  const toggleExpanded = (scannerId) => {
    setExpandedId((prev) => (prev === scannerId ? null : scannerId));
  };
    ...
    <Screen style={styles.screen} contentStyle={styles.container} header={<MobileHeader />}>
      <View style={styles.tabsRow}>
        <Pressable style={[styles.tabButton, styles.tabActive]}>
          <AppText style={[styles.tabText, styles.tabTextActive]}>Escaneres</AppText>
        </Pressable>
        <Pressable style={styles.tabButton} onPress={() => navigation.navigate("OrganizerSellers")}>
          <AppText style={styles.tabText}>Vendedores</AppText>
        </Pressable>
      </View>
      ...
      <View style={styles.listCard}>
        <View style={styles.listHeader}>
          ...
          <View style={styles.countChip}>
            <AppText style={styles.countText}>{scanners.length} escaneres</AppText>
          </View>
        </View>
        {scanners.length === 0 ? (
          <AppText style={styles.emptyText}>Sin escaneres.</AppText>
        ) : (
          scanners.map((scanner, index) => {
            const scannerId = resolveScannerId(scanner) || String(index);
            const isExpanded = expandedId === scannerId;
            const statusLabel = resolveStatus(scanner);
            const statusVariant = resolveStatusVariant(statusLabel);
            return (
              <View key={scannerId} style={styles.scannerCard}>
                <Pressable style={styles.scannerRow} onPress={() => toggleExpanded(scannerId)}>
                  <View style={styles.scannerInfo}>
                    ...
                    <View>
                      <AppText weight="semiBold">{scanner.email || "Scanner"}</AppText>
                      <AppText style={styles.metaText}>{scanner.name || "-"}</AppText>
                    </View>
                  </View>
                ...
                {isExpanded ? (
                  <View style={styles.scannerDetails}>
                    {[
                      { label: "NOMBRE", value: scanner.name || "-" },
                      { label: "CONTRASENA", value: scanner.password || "-" },
                      { label: "EVENTO", value: resolveEventName(scanner) },
                    ...
                    <View style={styles.detailActions}>
                      ...
                      <Pressable style={styles.iconButton} onPress={() => onDelete(scanner._id)}>
