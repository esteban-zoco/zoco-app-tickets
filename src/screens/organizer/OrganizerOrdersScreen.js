...
import { organizerListOrdersApi } from "../../services/api";
import { formatCurrency, formatDate } from "../../utils/format";
import { useAuth } from "../../store/AuthContext";

const OrganizerOrdersScreen = () => {
  const { state } = useAuth();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setIsLoading(true);
        const res = await organizerListOrdersApi();
      ...
      <View style={styles.container}>
        {orders.map((order) => (
          <View key={order._id} style={styles.card}>
            ...
            <AppText style={styles.meta}>{order?.buyer?.name || order?.name}</AppText>
...
export default OrganizerOrdersScreen;
